...
if ($id_owner) {
    try {
        ...
        if ($stmt->execute()) {
            ...
            header("Location: liste_owners.php?message=success&content=Le propriétaire a été activé avec succès.");
            exit();
        } else {
            // Redirigez avec un message d'erreur
            header("Location: liste_owners.php?message=error&content=Erreur lors de l'activation du propriétaire.");
            exit();
        }
